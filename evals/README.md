...
但仍能抓住最终答案结构和措辞回归。Terminal guard 边界用 `kind: ask_spec` + `mock_control`
给 classifier 一个受控结构化输出，然后调用真实 `NLEngine.ask` 终止路径；这避免外部
provider 不稳定，也避免用 hard-coded payload 冒充通过。不要用 hard-coded fixture 替代会声明
证据或 caveat 的数据路径。
