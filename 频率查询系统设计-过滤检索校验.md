...
> **"某个问题在某些产品/公司上的发生频率"**（如 _无菌问题在某家药厂的产品上出现了多少次_）。
...
> 唯一必须可靠的硬过滤是来源记录的结构化字段（如 `recalling_firm`/`classification`）；分类/抽取字段只是"可选加速器"，缺失或不准都不会让计数崩。

---

## 1. 最容易踩的坑：把它当 RAG 做

很多人一听"用自然语言在海量记录里找问题"，就直接套标准 RAG：
...
- top-K 只给"最像的 N 条"，但用户要的是"**一共发生了多少次、在哪些产品上分布如何**" —— 这需要对**全量**匹配记录做 recall，top-K 给不了。
...
只要这三件事分开、取**并集再校验**，**任何单个字段（含粗分类标签）缺失或不准，都不会让结果崩** —— 因为完整性不依赖任何单一字段。

---

## 3. 系统架构：两条路径 + 一个路由器

核心思想：**能离线预计算的，绝不在线算。** PLAN.md §4 的可选富化层产出的结构化标签，正是这套系统的地基。
...
flowchart TD
    ...
    P1 --> ANS[答案合成<br/>精确计数 + 分维度分布 + 证据 recall_number]
    P2 --> ANS
...
```sql
SELECT recalling_firm, COUNT(*) AS cnt
FROM drug_enforcement
WHERE hazard_type = 'sterility'        -- 离线打好的细粒度标签
...
1. **元数据预过滤**：用 `recalling_firm`/`classification` + 时间，把全量砍到候选几万条。
2. **混合召回**：BM25（精确术语，如药名/NDC/`NDMA`/`nitrosamine`）**+ 向量**（同义/改写），取并集缩到几千条。
...
## 4. 关键设计决策：不要把粗分类标签当"硬门槛"

核心：**粗分类（L1）标签不是 gate，只是可选加速器。**

### 4.1 过滤层级（按可靠性排序）

```mermaid
flowchart TD
    A[全量召回记录] --> B[第1层: 按 recalling_firm/classification 过滤<br/>结构化 · 近100%可靠 → 必做]
    B --> C[候选: 几万条]
    C --> D[第2层: 混合检索<br/>BM25 + 向量 · query='非无菌 / 污染']
    D --> E[候选 ~千条]
    E --> F[第3层: LLM/cross-encoder 逐条校验]
    F --> G[计数 + 证据]

    C -.可选加速.-> H[若 hazard_type / 抽取字段 高置信<br/>→ 进一步缩窄 or 排序加权]
    H -.-> D
```

- **唯一必须可靠、可硬过滤的是源头结构化字段**（如 `recalling_firm`/`classification`）：光靠它就把全量 → 候选几万条，足够后续检索跑得动。
- **粗分类标签缺失或不准 → 最坏只是少了一次加速**，绝不会把本该计入的记录错误排除。

### 4.2 "只有关键信息抽取字段、没有问题分类"怎么办？

**用你信得过的抽取字段来 narrow，反而比用粗 L1 更好：**

- `非无菌` 这个 query，本就该映射到 **`hazard_type = sterility`** 这种细粒度抽取字段，而不是去赌粗分类里有没有对应大类。
- 抽取字段**粒度更细、更贴近用户问法**，是更优的缩窄信号。
- 即便某些记录 hazard_type 漏抽，第 2 层的 **向量 + BM25 仍会在候选全量里捞回来**（recall 兜底）。**漏抽不会变成漏算。**
...
| 字段置信度 | 用法 |
| --- | --- |
| 高（如 `recalling_firm`，或高置信 `hazard_type`） | **hard filter**：直接缩小集合 |
| 中 | **soft boost**：不排除，只在排序里加权 |
| 低 / 缺失 | **完全不用它过滤**，纯靠检索召回 |

> 结果：任何分类/抽取字段不准或为空，**最坏只是少一次加速，绝不误排除应计入的记录**。
> 计数完整性始终由"源头字段硬过滤 + 全量混合检索 + 逐条校验"这条主干保证。
...
| 关注点 | 做法 |
| --- | --- |
| **同义/缩写** | openFDA 文本为英文；用嵌入桥接口语→术语（"too strong"→superpotent）、化学名↔通用名；必要时查询扩展 |
| **成本** | 嵌入、分类全部**离线批处理**；在线只在候选集（千级）上算；高频 query 缓存 |
| **延迟** | 元数据预过滤 + ANN（HNSW），先缩空间再算语义 |
| **防幻觉 / 可信** | 答案必须**附真实记录 ID（recall_number）作证据**；计数来自 SQL/检索结果，**绝不让 LLM 自己编数字** |
...
- 关键词：**Postgres FTS**（本项目）或 rank-bm25 / OpenSearch
- 结构化聚合：**PostgreSQL**（本项目）→ 生产可换 Snowflake
- 编排：async Python + OpenAI；chatbot 层 FastAPI + 静态 UI
...
1. **先做路径1 的 text-to-SQL chatbot**：做"自然语言问召回分布"对话界面（FastAPI + 静态 UI）。能回答"原因 X 在公司 A/B/C 上各多少次、趋势如何"，**带柱状图 + 证据 recall_number**。这一步已是一个完整可演示的 AIE/MLE 项目。
...
## 7. Scalability / 跨域泛化（从药品召回到任意数据）

> 核心追问：底层数据从药品召回换成手机订单、或别的库时，第一层过滤器怎么办？
> 答案：**第一层从来不是"产品过滤器"，而是"高来源可靠性的结构化维度过滤器"**；
> "哪些字段可硬过滤"不写死在代码里，而是一份**由 schema 自动剖析生成、HITL 确认的语义层配置**。
> 引擎只认 `dimension / measure / text_field` 三种抽象，不认药品召回还是手机订单。

### 7.1 按"来源可靠性 (provenance)"给字段分层，而非按名字

`recalling_firm` 只是"药品召回域里恰好最可靠的维度"。换域它可能变成 `SKU / 门店 / 渠道 / 订单状态`。能泛化的判据是 **provenance**——这字段是记录生成那一刻就被记录的事实，还是事后推断的：

| Tier | 定义（看来源，不看名字） | 药品召回 | 手机订单 | 过滤策略 |
| --- | --- | --- | --- | --- |
| **A — 源头记录的结构化事实** | 创建时即录入/生成，**非推断** | recalling_firm, classification, state, date | SKU, 门店, 渠道, 订单状态, 下单时间 | **hard filter**（永远安全） |
| **B — LLM 抽取/推断字段** | 从文本里推出来 | hazard_type, failure_mode | 退货原因, 情感, 投诉类型 | **置信度感知**（高→硬过滤，否则软加权） |
| **C — 自由文本** | 原始 narrative / 评论 | reason_for_recall, product_description | 用户评价 / 客服对话 | 混合检索兜底召回 |
...
class QuerySpec(BaseModel):
    filters: list[Filter]
    metric: Literal["order_count", "return_count"]   # 来自语义层 measures
...
## 9. 落地决策（v1 · drug/enforcement，2026-06-25 定稿）

> 把上面的通用架构落到当前数据集的具体选择。实时进度见 [PROGRESS.md](PROGRESS.md)。

### 9.1 嵌入与存储（已落地，多数据源就绪）
- 嵌入 `reason_for_recall`（"为什么召回"）**和** `product_description`（"什么产品"）两个字段；模型 OpenAI `text-embedding-3-small`（1536 维，~$0.05）。
- 存到**独立、多数据源**表 `embeddings`，主键 `(source, source_id, field)`，**每条记录每字段一行**——**不**在源表上加向量列；一个 HNSW + 一个 GIN 索引覆盖所有源/字段，**加新 FDA 数据集 = `src/embed.py` 里加一条 `SOURCES`**，不改表结构：

```sql
CREATE TABLE embeddings (
  source       text NOT NULL,       -- 源表名，如 'drug_enforcement'
  source_id    text NOT NULL,       -- 该源的天然键（如 recall_number）
  field        text NOT NULL,       -- 'reason_for_recall' | 'product_description'
  content      text NOT NULL,       -- 被嵌入的原文（供 FTS / 重排 / 展示）
  content_hash text NOT NULL,       -- 增量重嵌：文本变了才重算
  embedding    vector(1536),
  content_tsv  tsvector,            -- 关键词那一路（Postgres FTS）
  PRIMARY KEY (source, source_id, field)
);
-- HNSW（向量） + GIN（content_tsv 全文）两个索引；列含义见 sql/005 的 COMMENT
```

- 字段很短（reason 均长 109 / p95 297；product 均长 188 / p95 320 字符）→ **不切块**，整字段嵌入。
- **增量重嵌**：每次只嵌 `content_hash` 缺失或变化的行，对应 ingest 的 `--since auto`，幂等省钱。

### 9.2 检索（混合，留在 Postgres）
- 硬过滤复用现有 `QuerySpec.filters`（Tier A 列）缩小候选 → 向量 `<=>` ⊕ Postgres FTS `ts_rank` 用 **RRF** 融合 → top-K。
- 关键词那一路 **v1 用 Postgres 内置 FTS**（带词干化、单库）；若评估发现 keyword 召回不足，再升级到真 BM25（`pg_search`）。

### 9.3 路由
- `QuerySpec` 增加 `semantic_query` 字段：硬事实进 `filters`，模糊概念进 `semantic_query`（**禁用 `ilike` 关键词匹配**）。有 `semantic_query` → 路径 2。

### 9.4 计数 vs 检索（诚实性）
- v1 的 "show me …" 返回排序 **top-10**（可分页），不再硬砍 5 条。
- 语义"有多少条"= **估计问题**，留到**校验层（2.4）**：按相似度阈值取全集 → LLM 逐条判定 → 计数 + 阈值下的精度/召回 + 置信区间。

### 9.5 可观测 / 评估
- 每次 `/ask` 落 `query_log`（Postgres）：trace_id / question / spec(jsonb) / sql / row_count / latency / tokens / cost / repaired——既是可观测性，又是 eval 数据集与审计。再接 **Langfuse** 看 trace + 跑 eval 实验。
- 黄金集优先**确定性断言**（如 "sterility-related" 必须产出 `semantic_query` 而非 `ilike`）；忠实度等模糊维度才用 LLM-as-judge。

### 9.6 母公司用例 = Phase 3 capstone（Path 2 之后）
- 品牌 → 母公司是 **`[inferred]`**（LLM 世界知识 / Wikidata / FDA NDC labeler，标注 + 让用户确认）。
- 母公司 → 库内 `recalling_firm` 集合是**实体解析**：`pg_trgm` 模糊 + 已知子公司展开 + LLM 核验（firm 名碎片化，1,634 个，Pfizer/Teva/McNeil 散落在多种写法与子公司下）。
- 化妆品**基本超范围**：openFDA 只有化妆品**不良事件**（`/food/event`），无干净召回端点；架构可平移到 `device`/`food/enforcement`。
