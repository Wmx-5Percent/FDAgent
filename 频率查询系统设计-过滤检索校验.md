...
- 存到**独立表** `recall_embeddings`，**每条召回每字段一行**（用 `field` 区分），**不**在 `drug_enforcement` 上加向量列——一个 HNSW 索引覆盖全字段，加新字段不改表结构：
...
CREATE TABLE recall_embeddings (
  ...
  PRIMARY KEY (recall_number, field)
);
-- HNSW（向量） + GIN（content_tsv 全文）两个索引
```
