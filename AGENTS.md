...
- **Database**: PostgreSQL (Postgres.app) database `fda`, `pgvector` 0.8.1 enabled. DSN `postgresql://localhost:5432/fda` (override via `DATABASE_URL`).
...
- **Inspect data**: `psql -d fda -c "\dt"` ; query JSONB via `raw->>'field'`.
...
- **IP safety**: real company data is git-ignored and **never** committed (see
  [数据敏感性分析与合规说明.md](数据敏感性分析与合规说明.md)). Only public-domain
