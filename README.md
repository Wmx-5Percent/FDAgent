...
- **Raw firm exposure v0** — deterministic SQL over FDA `recalling_firm` strings ranks raw firm names by recall count or `raw_severity_v0` score (`3 * Class I + 2 * Class II + 1 * Class III`), with class breakdowns, top reason category, evidence, and caveats that this is not parent-group normalized and not a safety verdict.
