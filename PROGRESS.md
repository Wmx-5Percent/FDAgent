...
## Now
- **State:** Path 1 served + containerized. **Path 2.4 semantic validation/counting, Phase 3/4 sidecar foundations, Phase 4 taxonomy v1 exact counts, Frontend v2, and the serving-path OpenAI-compatible provider gateway are in place** — `/ask` can route fuzzy concepts through retrieval → LLM yes/no validation → estimated counts with evidence/confidence, and known taxonomy categories can use `recall_label` for exact counts; chat / structured-output calls can use `LLM_PROVIDER=openrouter`; embeddings stay on a separate provider/model boundary with explicit FTS-only fallback when embedding credentials/quota are unavailable.
...
14. **Phase 4 classification + exact taxonomy counts** — [sql/007_taxonomy.sql](sql/007_taxonomy.sql) + [src/classify/](src/classify/) provide sidecar taxonomy tables, frozen taxonomy v1, dry-run-first seed/backfill/audit CLIs, and `recall_label` population workflows. [src/analytics.py](src/analytics.py) / [src/nl_query.py](src/nl_query.py) can now route known recall-reason categories (e.g. `sterility_assurance`) to exact SQL counts/group-bys instead of semantic estimates; `/ask` returns normal scalar/distribution payloads with taxonomy metadata. Raw `recalling_firm` is still unnormalized until the firm track is wired.
...
3. **Phase 4 follow-up — taxonomy quality loop.**
   - Exact taxonomy counts are wired for known v1 categories. Next taxonomy work is quality/governance: inspect low-confidence/other residuals with [src/classify/discover.py](src/classify/discover.py), refine v1/v2 categories if needed, and calibrate LLM/rule labels against a small reviewed set.
   - **Done when:** residual reports produce reviewable candidate categories and label quality metrics are tracked before promoting taxonomy v2.
